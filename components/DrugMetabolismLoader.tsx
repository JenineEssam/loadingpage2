...
export default function DrugMetabolismLoader() {
  ...
  const ColorfulSea = ({ active = false, gender = "female" }) => {
    ...
    return (
      <motion.div 
        className="absolute bottom-0 left-0 right-0 h-full overflow-hidden"
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden">
      ...
      <motion.div 
        className="absolute top-8 sm:top-16 left-0 right-0 text-center z-10 px-4"
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 1.5, ease: "easeOut" }}
      >
        <motion.h1 
          className="text-amber-900 text-3xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-3 sm:mb-6 mx-auto"
          style={{
            textShadow: '0 0 25px rgba(217, 119, 6, 0.4)'
          }}
        >
          Pharmacokinetic Visualization
        </motion.h1>
        <div className="flex items-center justify-center space-x-3 sm:space-x-5 mx-auto mb-4 sm:mb-8">
          <Droplets className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600" />
          <p className="text-amber-700 text-sm sm:text-lg lg:text-xl tracking-wide uppercase font-semibold">
